...
class alexnet:
    def __init__(self, lr, dropout_rate, width, batch_size, weight_decay, train=True):
        ...
        if train:
            ...
            self.model.load_weights('cifar10alexnet.h5')
    ...
    def train(self,model):
        ...
        historytemp = model.fit_generator(datagen.flow(x_train, y_train,
                            ...
                            epochs=maxepoches,validation_data=(x_test, y_test),callbacks=[reduce_lr,save_weights.save_weights(fname='rawdata/alexnet_1_tanh/'+str(self.lr)+'_'+str(self.dropout_rate)+'_'+str(self.width)+'_'+str(self.batch_size)+'_'+str(self.weight_decay)+'_'+str(self.act))],verbose=2)
        return model
            ...
            for width in width_:
                for batch_size in batch_size_:
                    for lr in lr_:
                        ...
                        print('#############################')
                        print('#############################')
                        model = alexnet(lr, drop_rate, width, batch_size, weight_decay)                       